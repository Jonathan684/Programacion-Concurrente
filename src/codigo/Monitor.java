...
public class Monitor {
	...
	public boolean dispararTransicion(int T_Disparar) {
	    ...
	    while(k){
			...
			{   
				boolean b;
				b = cola.agregar(T_Disparar);
				//System.out.println("b:"+b+" T_Disparar:"+(T_Disparar+1));
				if(b)
					{
					...
					consola.registrarDisparo("* No Encolar: T"+ (T_Disparar+1),1);
					//k =false;
					mutex._release();
					return false;
				}
	...
	/**
	 * Calcula la operacion AND entre los que estan en la cola y las transiciones que estan en sensibilidas
	 * @return 
	 * 			m : Matriz con transiciones en la cola y sensibilizadas
	 */
